...
| `dist-workspace.toml`       | [cargo-dist](https://github.com/axodotdev/cargo-dist) release targets, CI backend, pinned `dist` version |
| `.github/workflows/release.yml` | Autogenerated by `dist generate`; tag-driven GitHub Releases + PR `dist plan` |
...
After editing `dist-workspace.toml`, run `dist generate` and commit the workflow diff. `[profile.dist]` in `Cargo.toml` is the release profile `dist build` uses.
...
`[.github/workflows/release.yml](.github/workflows/release.yml)` runs on matching version tags and on pull requests (plan-only). It is owned by cargo-dist; do not hand-edit unless you know what you are changing—prefer `dist generate`.
